  ...
  cron.schedule('0 8 * * *', async () => {
    try {
      ...
      const users = await User.findAll({
        where: { frecuencia_cobro: ['quincenal', 'mensual'] },
      });

      for (const user of users) {
        const esDiaDeCobro = esCobro(user.frecuencia_cobro, user.dia_cobro, dayOfMonth);
        if (!esDiaDeCobro) continue;
        ...
        const fmt = n => `RD$${n.toLocaleString('es-DO', { minimumFractionDigits: 0 })}`;

        await sendPushToUser(user.id, {
          title: '💸 Día de cobro — FinLibre',
          body: `Hoy cobras, ${user.nombre.split(' ')[0]}. Tienes ${deudas.length} deuda${deudas.length > 1 ? 's' : ''} por ${fmt(totalCuotas)}/mes.`,
          icon: '/icon-192.png',
          badge: '/badge-72.png',
          url: '/app/deudas',
          tag: 'cobro-reminder',
...
function esCobro(frecuencia, diaCobro, hoy) {
